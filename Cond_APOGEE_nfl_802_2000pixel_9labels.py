...
# +
spectra = np.loadtxt('./data/apogee_batch_Xtrain_full.csv')[:, :2000]
spectra = spectra.T
...
labels = np.loadtxt("./data/apogee_batch_ytrain_full.csv")[:2000]
labels.shape

#conditioning on teff, logg, vturb, vmac, ch, nh, oh, cah, feh
y = np.array([labels[:, 0], labels[:, 1], labels[:, 2], labels[:, 3], labels[:, 4], labels[:, 5], \
              labels[:, 10], labels[:, 13],  labels[:, 18]]).T

y[:, 0] = y[:, 0]/1000

x = spectra
y = torch.tensor(y, dtype=torch.float32).reshape(-1, 9)
print(y.shape)
...
#save checkpoint...
np.savetxt('cond_apogee_802_2000pixel_9labels.npy', loss_history)
torch.save({'epoch': n_epochs,
            'model_state_dict': model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict()
            }, 'cond_apogee_802_2000pixel_9labels.pth')
