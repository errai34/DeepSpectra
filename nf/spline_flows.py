...
class NSF_CL(nn.Module):
    ...
    def __init__(
        ...
        if context_features is not None:
            ...
            self.g1 = base_network(context_features, (3 * K - 1) * dim // 2, hidden_dim)
    ...
    def forward(self, x, context):
        ...
        if context is not None:
            out = (self.g1(context) * self.f1(lower)).reshape(
                -1, self.dim // 2, 3 * self.K - 1
            )
        else:
        ...
        if context is not None:
            out = (self.g2(context) * self.f2(upper)).reshape(
                -1, self.dim // 2, 3 * self.K - 1
            )
        else:
